...
import { Plan, PlanFrontmatter, PlanStep } from '../models/types';
...
export function parseFrontmatter(content: string): { meta: PlanFrontmatter; body: string } {
...
export function markStepDoneById(content: string, pointId: string): string {
  const lines = content.split('\n');
  return lines.map(line => {
    const m = line.match(STEP_REGEX);
    if (m && m[3] === pointId && m[2] !== 'x') {
      return line.replace(/\[ \]/, '[x]');
    }
    return line;
  }).join('\n');
...
// Validate plan has protocol frontmatter and at least one step
export function validatePlanProtocol(content: string): { valid: boolean; reason?: string } {
  const { meta, body } = parseFrontmatter(content);
  if (!meta.plangent) return { valid: false, reason: 'Missing frontmatter (plangent: 1)' };
  const steps = parsePlanSteps(body.length > 0 ? content : body);
  if (steps.length === 0) return { valid: false, reason: 'No checkbox steps found' };
  return { valid: true };
}

// Build the protocol reformat instruction for validation retry
export const PROTOCOL_REFORMAT_MSG =
  'Reformat the plan per the Plangent plan protocol: add YAML frontmatter with `plangent: 1`, `key`, `title`, `status`; and use `- [ ] (id) text` checkbox steps.';
