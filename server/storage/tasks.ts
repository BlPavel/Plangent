...
export function getTaskByKey(project_id: string, key: string): Task | null {
  const row = getDb().prepare('SELECT * FROM tasks WHERE project_id = ? AND key = ?').get(project_id, key) as Record<string, unknown> | undefined;
  return row ? parseTask(row) : null;
}
