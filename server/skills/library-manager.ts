...
import { LibraryItem, LibraryItemType } from '../models/types';
...
function mainContentDir(slug: string): string {
...
function getContentPath(item: LibraryItem): string {
  if (item.type === 'main') {
    return path.join(mainContentDir(item.slug), `${item.slug}.md`);
  }
...
function getOverridePath(item: LibraryItem, agentType: string): string {
  const dir = item.type === 'main'
    ? path.join(mainContentDir(item.slug), 'overrides')
